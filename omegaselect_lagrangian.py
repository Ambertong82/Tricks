...
def find_sign_changes_and_values(
        file1_path,
        file2_path,
        file3_path,
        output_path):
    ...
    for col_idx in omega_data.columns:
        ...
        for sc in sign_changes:
            if sc['sign_change'] == '-+':
                # 检查后续5行是否都为正（同一列）
                next_rows = range(
                    sc['row_idx'] + 1, min(sc['row_idx'] + 6, omega_data.index.max() + 1))
                if all(omega_data.loc[r, sc['col_idx']]
                       >= 0 for r in next_rows):
                    valid_neg_to_pos = sc
                    break
        ...
        if valid_neg_to_pos:
            ...
            if pos_to_neg_candidates:
                ...
                results.append({
                    ...
                    # 对应列的x位置
                    'x_position': x_positions[valid_neg_to_pos['col_idx']]
                })
    ...
    for result in results:
        try:
            ...
            print(
                f"Warning: Position ({result['neg_to_pos_row']}, {result['column_index']}) "
