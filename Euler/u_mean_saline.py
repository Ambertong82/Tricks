...
for i in range(len(times)):
    ...
    # 插值原始速度场（用于流线方向）
    uxi = griddata((X, Y), Ua_A[0], (xi, yi), method='nearest')  # Ux 分量
    uyi = griddata((X, Y), Ua_A[1], (xi, yi), method='nearest')  # Uy 分量
...
#def plot_velocity_vectors(X, Y, U_perturb, Ua_A, xi, yi, alpha_i, time_v, output_dir):
    ...
    """绘制原始速度流线图"""
    plt.figure(figsize=FIG_SIZE)
    uxi_scaled = np.clip(uxi, 0.1, 0.12)
