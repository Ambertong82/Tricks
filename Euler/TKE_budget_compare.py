...
# this script computes TKE budget terms from raw OF fields, without relying on pre-computed terms in OF. 
# It is used to compute the conservation terms
@dataclass
class TimeStepFields3D:
    ...
    grad_alpha_a: Optional[np.ndarray]
    grad_alpha_b: Optional[np.ndarray]
    grad_k: Optional[np.ndarray]
...
class TKEBudgetAnalyzer:
    def __init__(self):
        ...
        self.output_dir = "/home/amber/postpro/TKE_budget/tc3d_d23_0327_1conservation"
        self.times = [25]
    ...
    def _load_fields_3d(self, grid: Dict[str, np.ndarray], time_v: float) -> Optional[TimeStepFields3D]:
        ...
        # Prefer OpenFOAM gradients when available; fallback is handled in _compute_terms_3d.
        try:
            grad_u_raw = fluidfoam.readtensor(self.sol, time_dir, "grad(U.b)")
            grad_u = self._reshape_tensor_gradient(grad_u_raw, sort_idx, nx, ny, nz)
        except Exception as exc:
            if self.require_of_grad_u:
                print(f"Read failed for required grad(U.b) at t={time_v}: {exc}")
                return None
            print(f"Skip grad(U.b) at t={time_v}: {exc}")

        try:
            grad_alpha_a_raw = fluidfoam.readvector(self.sol, time_dir, "grad(alpha.a)")
            grad_alpha_a = self._reshape_vector_gradient(grad_alpha_a_raw, sort_idx, nx, ny, nz)
        except Exception as exc:
            print(f"Skip grad(alpha.a) at t={time_v}: {exc}")

        try:
            grad_alpha_b_raw = fluidfoam.readvector(self.sol, time_dir, "grad(alpha.b)")
            grad_alpha_b = self._reshape_vector_gradient(grad_alpha_b_raw, sort_idx, nx, ny, nz)
        except Exception as exc:
            print(f"Skip grad(alpha.b) at t={time_v}: {exc}")

        try:
            grad_k_raw = fluidfoam.readvector(self.sol, time_dir, "grad(k.b)")
            grad_k = self._reshape_vector_gradient(grad_k_raw, sort_idx, nx, ny, nz)
        except Exception as exc:
            print(f"Skip grad(k.b) at t={time_v}: {exc}")
        ...
        return TimeStepFields3D(
            ...
            grad_u=grad_u,
            grad_alpha_a=grad_alpha_a,
            grad_alpha_b=grad_alpha_b,
            grad_k=grad_k,
        )
    ...
    def _compute_terms_3d(
        ...
        grad_u = data.grad_u
        grad_alpha = data.grad_alpha_a if data.grad_alpha_a is not None else self._gradient_3d(data.alpha_a, data.x_axis, data.y_axis, data.z_axis)
        grad_beta = data.grad_alpha_b if data.grad_alpha_b is not None else self._gradient_3d(data.alpha_b, data.x_axis, data.y_axis, data.z_axis)
        hess_beta = self._hessian_3d(data.alpha_b, data.x_axis, data.y_axis, data.z_axis)
        grad_k = data.grad_k if data.grad_k is not None else self._gradient_3d(data.k, data.x_axis, data.y_axis, data.z_axis)
        ...
        ## convectionsecond
        conv_flux = self.rho_w * data.alpha_b * data.k
        divU = np.einsum("ii...->...", grad_u)
        convection2 = divU * conv_flux

        ## production
        production =  self.rho_w *np.einsum("ij...,ij...->...", sigma_ij, grad_u)
        ...
        Summ_right = -convection2 + production + density_gradient + diffusion + dissipation + drag1 + drag2 + drag3

        return {
            "convection2": np.nan_to_num(convection2, nan=0.0, posinf=0.0, neginf=0.0),
    ...
    def _save_outputs(self, time_v: float, df_curve: pd.DataFrame):
        ...
        ax4.set_title(f"G Term (Vertical Average) at t={time_v:.2f}s", fontsize=16)
        ax4.set_xlabel(r'$(x_f-x)/H$', fontsize=14)
        ax4.set_xlim(df_curve["x_dime"].max(), 0.0)
        ...
        fig4_path = os.path.join(self.output_dir, f"TKE_Budget_G_only_t{time_v:.2f}.png")
        fig4.savefig(fig4_path, dpi=300)
        plt.close(fig4)
        print(f"Saved Figure: {fig4_path}")

    def _grad_u_curve_components(self, grad_u: np.ndarray, y_axis: np.ndarray, head_idx: int) -> Dict[str, np.ndarray]:
        ...
        for i in range(3):
            for j in range(3):
                ...
                comp[key] = self._vertical_average(seg_2d[i, j], y_axis)
    ...
    def _save_grad_u_compare(
        ...
        of_comp = self._grad_u_curve_components(grad_u_of, y_axis, head_idx)
        py_comp = self._grad_u_curve_components(grad_u_py, y_axis, head_idx)
        ...
        csv_path = os.path.join(self.output_dir, f"gradU_compare_t{time_v:.2f}.csv")
        ...
        ax.set_title(f"gradU Comparison: OF vs Python at t={time_v:.2f}s", fontsize=16)
        ax.set_xlabel(r'$(x_f-x)/H$', fontsize=14)
        ax.set_xlim(compare_df["x_dime"].max(), 0.0)
        ax.set_ylabel("Vertical average", fontsize=14)
        ax.tick_params(axis="both", labelsize=12)
        ax.grid(True, linestyle="--", alpha=0.35)
        ...
        fig_path = os.path.join(self.output_dir, f"gradU_compare_t{time_v:.2f}.png")
        fig.savefig(fig_path, dpi=300)
        plt.close(fig)
        print(f"Saved Figure: {fig_path}")
